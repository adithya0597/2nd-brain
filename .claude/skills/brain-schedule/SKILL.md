...
name: brain-schedule
description: >
  Use this skill when the user wants to plan their week or build a multi-day
  schedule — any request that signals "help me allocate my time and energy
  across the coming days." This includes planning the week ahead, asking what
  to prioritize this week, wanting an energy-optimized schedule, requesting
  task distribution across days, or preparing a weekly game plan. Combines
  pending tasks, Notion deadlines, historical energy patterns from journal
  data, and ICOR dimension balance to produce a day-by-day allocation.
  Distinguished from the morning review (which focuses only on today, not the
  full week) and from simply reading a calendar (which is passive lookup, not
  active planning and energy-aware task distribution).
---
...
- If no specific time mentioned, use the current week (Monday through Sunday containing today)
- If "next" or "next week" is mentioned, use next Monday through Sunday
- If a specific date is given, use the Monday-Sunday week containing that date

Resolve today's date using `date +%Y-%m-%d`. Calculate `week_start` (Monday) and `week_end` (Sunday) dates.

### 2. Gather Energy Patterns from Journal History
Run against `data/brain.db` using `sqlite3` for historical energy and mood by day of week (last 90 days):
```sql
SELECT
...
Run against `data/brain.db` using `sqlite3` for all pending actions with ICOR context:
```sql
...
Run against `data/brain.db` using `sqlite3` for neglected areas needing recovery attention:
```sql
...
Use the output template in `references/output-template.md` to format the final schedule.
