...
description: >
  Use this skill when the user wants to improve their CLAUDE.md project
  instructions based on real usage patterns — any request that signals "my
  instructions could be better and I want data-driven suggestions." This
  includes auditing CLAUDE.md quality, finding instructions that keep getting
  violated, discovering missing rules that should be codified, pruning
  outdated guidance, or wanting to make Claude more effective on this project.
  Analyzes recent conversation history to surface violated instructions,
  missing patterns, and stale rules in both global and local CLAUDE.md files.
  Distinguished from handoff (which captures session state for continuity, not
  instruction quality) and from general code review (this targets CLAUDE.md
  specifically, not application code).
---
